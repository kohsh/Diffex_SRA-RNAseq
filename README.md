...
setwd(path.expand("/media/boris/kosar/apps/Analysis/STAR/Diffex-analysis/PBMC-AD_vs_PD") )

# Read in the data
covar <- read.delim("meta_df.txt", sep='\t', header=TRUE, stringsAsFactors=FALSE)
reads <- read.delim("ND_CPM.txt", sep='\t', header=TRUE, stringsAsFactors=FALSE)

# Filtering out lowly expressed genes based on sample groups
group <- paste(c(rep("AD_PBMC",22), rep("PBMC_PD",46)))
...
datTraits <- dplyr::select(covar, c('Sample', 'Project_Numeric', 'Strategy_Numeric', 'Condition', 'Age', 'Sex'))
head(datTraits)
